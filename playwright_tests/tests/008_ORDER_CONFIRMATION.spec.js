...
test('test', async ({ page }) => {
  ...
  await expect(deliveryRadio).toBeChecked({ timeout: 5000 });
  ...
  await expect(blikRadio).toBeChecked({ timeout: 5000 });
  ...
  await page.getByRole('link', { name: 'Przejdź do płatności', timeout: 15000 }).click(); // Zwiększony timeout
});