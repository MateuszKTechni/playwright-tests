...
test('test', async ({ page }) => {
  ...
  if (await addToCartButton.isVisible()) {
    ...
    await expect(page.getByText('Twój koszyk jest pusty')).toBeVisible({ timeout: 5000 });

  } else {
    console.log('Nie można było znaleźć przycisku "Dodaj do koszyka" w Teście 4. Sprawdź selektor!');
  }
});